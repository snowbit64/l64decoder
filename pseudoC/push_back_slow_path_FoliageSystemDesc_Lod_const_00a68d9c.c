// Ghidra Decompiler Output - AArch64 (64-bit)
// Function: __push_back_slow_path<FoliageSystemDesc::Lod_const&>
// Entry Point: 00a68d9c
// Size: 424 bytes
// Signature: void __thiscall __push_back_slow_path<FoliageSystemDesc::Lod_const&>(vector<FoliageSystemDesc::Lod,std::__ndk1::allocator<FoliageSystemDesc::Lod>> * this, Lod * param_1)


/* void std::__ndk1::vector<FoliageSystemDesc::Lod, std::__ndk1::allocator<FoliageSystemDesc::Lod>
   >::__push_back_slow_path<FoliageSystemDesc::Lod const&>(FoliageSystemDesc::Lod const&) */

void __thiscall
std::__ndk1::vector<FoliageSystemDesc::Lod,std::__ndk1::allocator<FoliageSystemDesc::Lod>>::
__push_back_slow_path<FoliageSystemDesc::Lod_const&>
          (vector<FoliageSystemDesc::Lod,std::__ndk1::allocator<FoliageSystemDesc::Lod>> *this,
          Lod *param_1)

{
  ulong uVar1;
  undefined8 *puVar2;
  long lVar3;
  void *pvVar4;
  long lVar5;
  undefined8 *puVar6;
  ulong uVar7;
  undefined8 *puVar8;
  ulong uVar9;
  undefined8 *puVar10;
  long lVar11;
  undefined8 *puVar12;
  undefined8 uVar13;
  undefined8 uVar14;
  undefined8 uVar15;
  
  lVar3 = tpidr_el0;
  lVar5 = *(long *)(lVar3 + 0x28);
  lVar11 = *(long *)(this + 8) - *(long *)this >> 6;
  uVar1 = lVar11 + 1;
  if (uVar1 >> 0x3a != 0) {
                    /* WARNING: Subroutine does not return */
    std::__ndk1::__vector_base_common<true>::__throw_length_error();
  }
  uVar7 = *(long *)(this + 0x10) - *(long *)this;
  uVar9 = (long)uVar7 >> 5;
  if (uVar1 <= uVar9) {
    uVar1 = uVar9;
  }
  if (0x7fffffffffffffbf < uVar7) {
    uVar1 = 0x3ffffffffffffff;
  }
  if (uVar1 == 0) {
    pvVar4 = (void *)0x0;
  }
  else {
    if (uVar1 >> 0x3a != 0) {
                    /* WARNING: Subroutine does not return */
      FUN_006d1f18("allocator<T>::allocate(size_t n) \'n\' exceeds maximum supported size");
    }
    pvVar4 = operator_new(uVar1 << 6);
  }
  puVar12 = (undefined8 *)((long)pvVar4 + lVar11 * 0x40);
  *puVar12 = *(undefined8 *)param_1;
                    /* try { // try from 00a68e48 to 00a68e4b has its CatchHandler @ 00a68f44 */
  std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>::
  basic_string((basic_string *)(puVar12 + 1));
  puVar6 = *(undefined8 **)this;
  puVar8 = *(undefined8 **)(this + 8);
  puVar2 = puVar12 + 8;
  uVar13 = *(undefined8 *)(param_1 + 0x20);
  uVar15 = *(undefined8 *)(param_1 + 0x35);
  uVar14 = *(undefined8 *)(param_1 + 0x2d);
  puVar12[5] = *(undefined8 *)(param_1 + 0x28);
  puVar12[4] = uVar13;
  *(undefined8 *)((long)puVar12 + 0x35) = uVar15;
  *(undefined8 *)((long)puVar12 + 0x2d) = uVar14;
  puVar10 = puVar6;
  if (puVar8 != puVar6) {
    do {
      puVar10 = puVar8 + -8;
      puVar12[-8] = *puVar10;
      uVar14 = puVar8[-6];
      uVar13 = puVar8[-7];
      puVar12[-5] = puVar8[-5];
      puVar12[-6] = uVar14;
      puVar12[-7] = uVar13;
      uVar13 = *(undefined8 *)((long)puVar8 + -0x13);
      puVar8[-6] = 0;
      puVar8[-5] = 0;
      uVar15 = puVar8[-3];
      uVar14 = puVar8[-4];
      puVar8[-7] = 0;
      *(undefined8 *)((long)puVar12 + -0xb) = *(undefined8 *)((long)puVar8 + -0xb);
      *(undefined8 *)((long)puVar12 + -0x13) = uVar13;
      puVar12[-3] = uVar15;
      puVar12[-4] = uVar14;
      puVar12 = puVar12 + -8;
      puVar8 = puVar10;
    } while (puVar10 != puVar6);
    puVar6 = *(undefined8 **)(this + 8);
    puVar10 = *(undefined8 **)this;
  }
  *(undefined8 **)this = puVar12;
  *(undefined8 **)(this + 8) = puVar2;
  *(void **)(this + 0x10) = (void *)((long)pvVar4 + uVar1 * 0x40);
  while (puVar2 = puVar6, puVar2 != puVar10) {
    puVar6 = puVar2 + -8;
    if ((*(byte *)(puVar2 + -7) & 1) != 0) {
      operator_delete((void *)puVar2[-5]);
    }
  }
  if (puVar10 != (undefined8 *)0x0) {
    operator_delete(puVar10);
  }
  if (*(long *)(lVar3 + 0x28) == lVar5) {
    return;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}


