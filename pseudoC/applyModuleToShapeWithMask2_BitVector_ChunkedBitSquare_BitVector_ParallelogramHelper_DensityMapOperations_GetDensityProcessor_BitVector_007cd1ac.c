// Ghidra Decompiler Output - AArch64 (64-bit)
// Function: applyModuleToShapeWithMask2<BitVector,ChunkedBitSquare,BitVector,ParallelogramHelper,DensityMapOperations::GetDensityProcessor<BitVector>>
// Entry Point: 007cd1ac
// Size: 4244 bytes
// Signature: bool __cdecl applyModuleToShapeWithMask2<BitVector,ChunkedBitSquare,BitVector,ParallelogramHelper,DensityMapOperations::GetDensityProcessor<BitVector>>(IDensityPixelSource * param_1, BitVector * param_2, ChunkedBitSquare * param_3, BitVector * param_4, ParallelogramHelper * param_5, GetDensityProcessor * param_6, DensityMapFilter * param_7, DensityMapFilter * param_8)


/* bool DensityMapModifier::applyModuleToShapeWithMask2<BitVector, ChunkedBitSquare, BitVector,
   ParallelogramHelper, DensityMapOperations::GetDensityProcessor<BitVector> >(IDensityPixelSource*,
   BitVector*, ChunkedBitSquare*, BitVector*, ParallelogramHelper&,
   DensityMapOperations::GetDensityProcessor<BitVector>&, DensityMapFilter const&, DensityMapFilter
   const&) */

bool DensityMapModifier::
     applyModuleToShapeWithMask2<BitVector,ChunkedBitSquare,BitVector,ParallelogramHelper,DensityMapOperations::GetDensityProcessor<BitVector>>
               (IDensityPixelSource *param_1,BitVector *param_2,ChunkedBitSquare *param_3,
               BitVector *param_4,ParallelogramHelper *param_5,GetDensityProcessor *param_6,
               DensityMapFilter *param_7,DensityMapFilter *param_8)

{
  uint *puVar1;
  uint uVar2;
  uint uVar3;
  uint uVar4;
  uint uVar5;
  int iVar6;
  uint uVar7;
  int iVar8;
  uint uVar9;
  int iVar10;
  int iVar11;
  DensityMapFilter DVar12;
  DensityMapFilter DVar13;
  GetDensityProcessor GVar14;
  int iVar15;
  uint uVar16;
  uint uVar17;
  uint uVar18;
  uint uVar19;
  bool bVar20;
  uint uVar21;
  uint uVar22;
  uint uVar23;
  uint uVar24;
  uint uVar25;
  uint uVar26;
  uint uVar27;
  uint uVar28;
  uint uVar29;
  uint uVar30;
  uint uVar31;
  int iVar32;
  uint uVar33;
  int iVar34;
  int iVar35;
  code **ppcVar36;
  ulong uVar37;
  int *piVar38;
  int *piVar39;
  long lVar40;
  uint uVar41;
  uint uVar42;
  uint uVar43;
  uint uVar44;
  long lVar45;
  uint uVar46;
  long *plVar47;
  ulong uVar48;
  undefined4 *puVar49;
  int iVar50;
  undefined2 uVar51;
  undefined8 uVar52;
  uint local_160;
  uint local_15c;
  uint local_128;
  uint local_110;
  uint local_c0;
  uint local_bc;
  uint local_a8;
  uint local_94;
  uint local_6c;
  
  uVar21 = (**(code **)(*(long *)param_1 + 0x18))();
  uVar22 = (**(code **)(*(long *)param_1 + 0x20))(param_1);
  uVar23 = (**(code **)(*(long *)param_1 + 0x30))(param_1);
  plVar47 = *(long **)param_7;
  uVar24 = (**(code **)(*plVar47 + 0x18))(plVar47);
  uVar25 = (**(code **)(*plVar47 + 0x20))(plVar47);
  (**(code **)(*plVar47 + 0x30))(plVar47);
  (**(code **)(**(long **)param_7 + 0x28))();
  uVar4 = *(uint *)(param_7 + 0x10);
  uVar16 = ~(-1 << (ulong)(*(uint *)(param_7 + 0x14) & 0x1f));
  uVar26 = (**(code **)(**(long **)param_7 + 0x28))();
  iVar35 = *(int *)(param_7 + 8);
  uVar5 = *(uint *)(param_7 + 0xc);
  plVar47 = *(long **)param_8;
  iVar32 = *(int *)(param_7 + 0x18);
  iVar6 = *(int *)(param_7 + 0x1c);
  DVar12 = param_7[0x20];
  uVar7 = *(uint *)(param_7 + 0x24);
  uVar17 = ~(-1 << (ulong)(uVar26 & 0x1f));
  uVar27 = (**(code **)(*plVar47 + 0x18))(plVar47);
  uVar28 = (**(code **)(*plVar47 + 0x20))(plVar47);
  uVar29 = (**(code **)(*plVar47 + 0x30))(plVar47);
  (**(code **)(**(long **)param_8 + 0x28))();
  uVar26 = *(uint *)(param_8 + 0x10);
  uVar18 = ~(-1 << (ulong)(*(uint *)(param_8 + 0x14) & 0x1f));
  uVar30 = (**(code **)(**(long **)param_8 + 0x28))();
  iVar8 = *(int *)(param_8 + 8);
  DVar13 = param_8[0x20];
  uVar9 = *(uint *)(param_8 + 0xc);
  iVar10 = *(int *)(param_8 + 0x18);
  iVar11 = *(int *)(param_8 + 0x1c);
  uVar19 = ~(-1 << (ulong)(uVar30 & 0x1f));
  uVar30 = *(uint *)(param_8 + 0x24);
  if ((((uVar21 < uVar24) || (uVar22 < uVar25)) || (uVar21 < uVar27)) || (uVar22 < uVar28)) {
    if (((uVar24 < uVar21) || (uVar25 < uVar22)) || ((uVar27 < uVar21 || (uVar28 < uVar22)))) {
      if (((LogManager::getInstance()::singletonLogManager & 1) == 0) &&
         (iVar35 = __cxa_guard_acquire(&LogManager::getInstance()::singletonLogManager), iVar35 != 0
         )) {
                    /* try { // try from 007ce208 to 007ce213 has its CatchHandler @ 007ce240 */
        LogManager::LogManager((LogManager *)&LogManager::getInstance()::singletonLogManager);
        __cxa_atexit(LogManager::~LogManager,&LogManager::getInstance()::singletonLogManager,
                     &PTR_LOOP_00fd8de0);
        __cxa_guard_release(&LogManager::getInstance()::singletonLogManager);
      }
      LogManager::errorf((char *)&LogManager::getInstance()::singletonLogManager,
                         "DensityMapModifier: masks must either be scaled up or scaled down in both directions - mask size %ux%u, mask2 size %ux%u and density map size %ux%u are incompatible\n"
                         ,(ulong)uVar24,(ulong)uVar25,(ulong)uVar27,(ulong)uVar28,(ulong)uVar21,
                         (ulong)uVar22);
      return false;
    }
    uVar43 = 0;
    if (uVar22 != 0) {
      uVar43 = uVar25 / uVar22;
    }
    uVar25 = 0;
    if (uVar21 != 0) {
      uVar25 = uVar24 / uVar21;
    }
    uVar52 = NEON_cnt((ulong)uVar25,1);
    uVar51 = NEON_uaddlv(uVar52,1);
    if ((int)CONCAT62((int6)((ulong)uVar52 >> 0x10),uVar51) == 1) {
      bVar20 = (uVar25 & 0xffff0000) != 0;
      uVar24 = uVar25 >> 0x10;
      if (!bVar20) {
        uVar24 = uVar25;
      }
      uVar42 = (uint)bVar20;
      uVar31 = uVar42 << 4 | 8;
      uVar41 = uVar24 >> 8;
      if (uVar24 < 0x100) {
        uVar31 = uVar42 << 4;
        uVar41 = uVar24;
      }
      uVar24 = uVar31 | 4;
      uVar42 = uVar41 >> 4;
      if (uVar41 < 0x10) {
        uVar24 = uVar31;
        uVar42 = uVar41;
      }
      uVar31 = uVar24 | 2;
      uVar41 = uVar42 >> 2;
      if (uVar42 < 4) {
        uVar31 = uVar24;
        uVar41 = uVar42;
      }
      local_110 = (uVar41 + uVar31) - 1;
    }
    else {
      local_110 = 0;
    }
    uVar52 = NEON_cnt((ulong)uVar43,1);
    uVar24 = 0;
    if (uVar21 != 0) {
      uVar24 = uVar27 / uVar21;
    }
    uVar51 = NEON_uaddlv(uVar52,1);
    if ((int)CONCAT62((int6)((ulong)uVar52 >> 0x10),uVar51) == 1) {
      bVar20 = (uVar43 & 0xffff0000) != 0;
      uVar31 = uVar43 >> 0x10;
      if (!bVar20) {
        uVar31 = uVar43;
      }
      uVar44 = (uint)bVar20;
      uVar41 = uVar44 << 4 | 8;
      uVar42 = uVar31 >> 8;
      if (uVar31 < 0x100) {
        uVar41 = uVar44 << 4;
        uVar42 = uVar31;
      }
      uVar31 = uVar41 | 4;
      uVar44 = uVar42 >> 4;
      if (uVar42 < 0x10) {
        uVar31 = uVar41;
        uVar44 = uVar42;
      }
      uVar41 = uVar31 | 2;
      uVar42 = uVar44 >> 2;
      if (uVar44 < 4) {
        uVar41 = uVar31;
        uVar42 = uVar44;
      }
      local_15c = (uVar42 + uVar41) - 1;
    }
    else {
      local_15c = 0;
    }
    uVar31 = 0;
    if (uVar22 != 0) {
      uVar31 = uVar28 / uVar22;
    }
    uVar52 = NEON_cnt((ulong)uVar24,1);
    uVar51 = NEON_uaddlv(uVar52,1);
    if ((int)CONCAT62((int6)((ulong)uVar52 >> 0x10),uVar51) == 1) {
      bVar20 = (uVar24 & 0xffff0000) != 0;
      uVar22 = uVar24 >> 0x10;
      if (!bVar20) {
        uVar22 = uVar24;
      }
      uVar42 = (uint)bVar20;
      uVar28 = uVar42 << 4 | 8;
      uVar41 = uVar22 >> 8;
      if (uVar22 < 0x100) {
        uVar28 = uVar42 << 4;
        uVar41 = uVar22;
      }
      uVar22 = uVar28 | 4;
      uVar42 = uVar41 >> 4;
      if (uVar41 < 0x10) {
        uVar22 = uVar28;
        uVar42 = uVar41;
      }
      uVar28 = uVar22 | 2;
      uVar41 = uVar42 >> 2;
      if (uVar42 < 4) {
        uVar28 = uVar22;
        uVar41 = uVar42;
      }
      local_128 = (uVar41 + uVar28) - 1;
    }
    else {
      local_128 = 0;
    }
    uVar52 = NEON_cnt((ulong)uVar31,1);
    uVar51 = NEON_uaddlv(uVar52,1);
    if ((int)CONCAT62((int6)((ulong)uVar52 >> 0x10),uVar51) == 1) {
      bVar20 = (uVar31 & 0xffff0000) != 0;
      uVar22 = uVar31 >> 0x10;
      if (!bVar20) {
        uVar22 = uVar31;
      }
      uVar42 = (uint)bVar20;
      uVar28 = uVar42 << 4 | 8;
      uVar41 = uVar22 >> 8;
      if (uVar22 < 0x100) {
        uVar28 = uVar42 << 4;
        uVar41 = uVar22;
      }
      uVar22 = uVar28 | 4;
      uVar42 = uVar41 >> 4;
      if (uVar41 < 0x10) {
        uVar22 = uVar28;
        uVar42 = uVar41;
      }
      uVar28 = uVar22 | 2;
      uVar41 = uVar42 >> 2;
      if (uVar42 < 4) {
        uVar28 = uVar22;
        uVar41 = uVar42;
      }
      local_160 = (uVar41 + uVar28) - 1;
    }
    else {
      local_160 = 0;
    }
    uVar22 = *(uint *)(param_5 + 0x48);
    if (uVar22 < *(uint *)(param_5 + 0x4c)) {
      if (uVar25 < 2) {
        uVar25 = 1;
      }
      if (uVar43 < 2) {
        uVar43 = 1;
      }
      if (uVar24 < 2) {
        uVar24 = 1;
      }
      if (uVar31 < 2) {
        uVar31 = 1;
      }
      do {
        ParallelogramHelper::startXIteration(param_5,uVar22);
        uVar28 = *(uint *)(param_5 + 0x58);
        if (uVar28 < *(uint *)(param_5 + 0x5c)) {
LAB_007cddd8:
          uVar41 = 0;
          uVar42 = uVar28 << (ulong)(local_110 & 0x1f);
          do {
            uVar44 = 0;
            uVar37 = (ulong)*(uint *)(param_3 + 0x34);
            uVar46 = uVar41 + (uVar22 << (ulong)(local_15c & 0x1f));
            uVar3 = uVar42;
            do {
              uVar2 = uVar44 + uVar42;
              iVar15 = (uVar46 >> 5) << (ulong)(*(uint *)param_3 & 0x1f);
              iVar50 = (int)uVar37;
              ppcVar36 = *(code ***)
                          (*(long *)(param_3 + 0x28) + (ulong)((iVar15 + (uVar2 >> 5)) * iVar50) * 8
                          );
              uVar2 = uVar2 & 0x1f | (uVar46 & 0x1f) << 5;
              uVar33 = (**ppcVar36)(ppcVar36,uVar2,0,*(undefined4 *)(param_3 + 0x3c));
              piVar38 = (int *)(param_6 + 4);
              if (*(uint *)(param_3 + 0x34) < 2) {
                uVar37 = (ulong)*(uint *)(param_3 + 0x34);
                if (iVar35 != 2) goto LAB_007cdf0c;
LAB_007cdf2c:
                uVar2 = uVar33 >> (ulong)(uVar4 & 0x1f) & uVar16;
                if (((int)uVar2 < iVar32) || (iVar6 < (int)uVar2)) {
                  if (DVar12 != (DensityMapFilter)0x0) goto LAB_007cdf58;
                }
                else if (DVar12 == (DensityMapFilter)0x0) {
LAB_007cdf58:
                  if ((uVar7 == 0) || ((uVar2 & uVar7) != 0)) {
                    uVar41 = 0;
                    uVar42 = uVar29 * (uVar27 * (uVar22 << (ulong)(local_160 & 0x1f)) +
                                      (uVar28 << (ulong)(local_128 & 0x1f)));
                    lVar40 = *(long *)param_4;
                    goto LAB_007cdfcc;
                  }
                }
              }
              else {
                uVar48 = 1;
                puVar49 = (undefined4 *)(param_3 + 0x44);
                do {
                  ppcVar36 = *(code ***)
                              (*(long *)(param_3 + 0x28) +
                              (ulong)(iVar50 * (iVar15 + (uVar3 >> 5)) + (int)uVar48) * 8);
                  iVar34 = (**ppcVar36)(ppcVar36,uVar2,0,*puVar49);
                  puVar1 = puVar49 + -1;
                  uVar48 = uVar48 + 1;
                  uVar37 = (ulong)*(uint *)(param_3 + 0x34);
                  puVar49 = puVar49 + 2;
                  uVar33 = iVar34 << (ulong)(*puVar1 & 0x1f) | uVar33;
                } while (uVar48 < uVar37);
                if (iVar35 == 2) goto LAB_007cdf2c;
LAB_007cdf0c:
                if ((iVar35 == 0) != ((uVar33 & uVar17) != uVar5)) goto LAB_007cdf2c;
              }
              uVar44 = uVar44 + 1;
              uVar3 = uVar3 + 1;
            } while (uVar44 != uVar25);
            uVar41 = uVar41 + 1;
          } while (uVar41 != uVar43);
          goto LAB_007cddbc;
        }
LAB_007cdd44:
        uVar22 = uVar22 + 1;
      } while (uVar22 < *(uint *)(param_5 + 0x4c));
    }
  }
  else {
    uVar43 = 0;
    if (uVar24 != 0) {
      uVar43 = uVar21 / uVar24;
    }
    uVar24 = 0;
    if (uVar25 != 0) {
      uVar24 = uVar22 / uVar25;
    }
    uVar52 = NEON_cnt((ulong)uVar43,1);
    uVar51 = NEON_uaddlv(uVar52,1);
    if ((int)CONCAT62((int6)((ulong)uVar52 >> 0x10),uVar51) == 1) {
      bVar20 = (uVar43 & 0xffff0000) != 0;
      uVar25 = uVar43 >> 0x10;
      if (!bVar20) {
        uVar25 = uVar43;
      }
      uVar41 = (uint)bVar20;
      uVar43 = uVar41 << 4 | 8;
      uVar31 = uVar25 >> 8;
      if (uVar25 < 0x100) {
        uVar43 = uVar41 << 4;
        uVar31 = uVar25;
      }
      uVar25 = uVar43 | 4;
      uVar41 = uVar31 >> 4;
      if (uVar31 < 0x10) {
        uVar25 = uVar43;
        uVar41 = uVar31;
      }
      uVar43 = uVar25 | 2;
      uVar31 = uVar41 >> 2;
      if (uVar41 < 4) {
        uVar43 = uVar25;
        uVar31 = uVar41;
      }
      local_6c = (uVar31 + uVar43) - 1;
    }
    else {
      local_6c = 0;
    }
    uVar52 = NEON_cnt((ulong)uVar24,1);
    uVar25 = 0;
    if (uVar27 != 0) {
      uVar25 = uVar21 / uVar27;
    }
    uVar51 = NEON_uaddlv(uVar52,1);
    if ((int)CONCAT62((int6)((ulong)uVar52 >> 0x10),uVar51) == 1) {
      bVar20 = (uVar24 & 0xffff0000) != 0;
      uVar43 = uVar24 >> 0x10;
      if (!bVar20) {
        uVar43 = uVar24;
      }
      uVar41 = (uint)bVar20;
      uVar24 = uVar41 << 4 | 8;
      uVar31 = uVar43 >> 8;
      if (uVar43 < 0x100) {
        uVar24 = uVar41 << 4;
        uVar31 = uVar43;
      }
      uVar43 = uVar24 | 4;
      uVar41 = uVar31 >> 4;
      if (uVar31 < 0x10) {
        uVar43 = uVar24;
        uVar41 = uVar31;
      }
      uVar24 = uVar43 | 2;
      uVar31 = uVar41 >> 2;
      if (uVar41 < 4) {
        uVar24 = uVar43;
        uVar31 = uVar41;
      }
      local_bc = (uVar31 + uVar24) - 1;
    }
    else {
      local_bc = 0;
    }
    uVar24 = 0;
    if (uVar28 != 0) {
      uVar24 = uVar22 / uVar28;
    }
    uVar52 = NEON_cnt((ulong)uVar25,1);
    uVar51 = NEON_uaddlv(uVar52,1);
    if ((int)CONCAT62((int6)((ulong)uVar52 >> 0x10),uVar51) == 1) {
      bVar20 = (uVar25 & 0xffff0000) != 0;
      uVar22 = uVar25 >> 0x10;
      if (!bVar20) {
        uVar22 = uVar25;
      }
      uVar43 = (uint)bVar20;
      uVar25 = uVar43 << 4 | 8;
      uVar28 = uVar22 >> 8;
      if (uVar22 < 0x100) {
        uVar25 = uVar43 << 4;
        uVar28 = uVar22;
      }
      uVar22 = uVar25 | 4;
      uVar43 = uVar28 >> 4;
      if (uVar28 < 0x10) {
        uVar22 = uVar25;
        uVar43 = uVar28;
      }
      uVar25 = uVar22 | 2;
      uVar28 = uVar43 >> 2;
      if (uVar43 < 4) {
        uVar25 = uVar22;
        uVar28 = uVar43;
      }
      local_94 = (uVar28 + uVar25) - 1;
    }
    else {
      local_94 = 0;
    }
    uVar52 = NEON_cnt((ulong)uVar24,1);
    uVar51 = NEON_uaddlv(uVar52,1);
    if ((int)CONCAT62((int6)((ulong)uVar52 >> 0x10),uVar51) == 1) {
      bVar20 = (uVar24 & 0xffff0000) != 0;
      uVar22 = uVar24 >> 0x10;
      if (!bVar20) {
        uVar22 = uVar24;
      }
      uVar28 = (uint)bVar20;
      uVar24 = uVar28 << 4 | 8;
      uVar25 = uVar22 >> 8;
      if (uVar22 < 0x100) {
        uVar24 = uVar28 << 4;
        uVar25 = uVar22;
      }
      uVar22 = uVar24 | 4;
      uVar28 = uVar25 >> 4;
      if (uVar25 < 0x10) {
        uVar22 = uVar24;
        uVar28 = uVar25;
      }
      uVar24 = uVar22 | 2;
      uVar25 = uVar28 >> 2;
      if (uVar28 < 4) {
        uVar24 = uVar22;
        uVar25 = uVar28;
      }
      local_c0 = (uVar25 + uVar24) - 1;
    }
    else {
      local_c0 = 0;
    }
    uVar22 = *(uint *)(param_5 + 0x48);
    if (uVar22 < *(uint *)(param_5 + 0x4c)) {
      piVar38 = (int *)(param_6 + 4);
      local_a8 = ~(-1 << (ulong)(uVar29 & 0x1f)) & 0xffff;
      do {
        ParallelogramHelper::startXIteration(param_5,uVar22);
        uVar24 = *(uint *)(param_5 + 0x58);
        if (uVar24 < *(uint *)(param_5 + 0x5c)) {
          uVar25 = uVar22 >> (ulong)(local_bc & 0x1f);
          uVar37 = (ulong)*(uint *)(param_3 + 0x34);
          do {
            uVar28 = uVar24 >> (ulong)(local_6c & 0x1f);
            uVar43 = (((uVar25 >> 5) << (ulong)(*(uint *)param_3 & 0x1f)) + (uVar28 >> 5)) *
                     (int)uVar37;
            ppcVar36 = *(code ***)(*(long *)(param_3 + 0x28) + (ulong)uVar43 * 8);
            uVar28 = uVar28 & 0x1f | (uVar25 & 0x1f) << 5;
            uVar31 = (**ppcVar36)(ppcVar36,uVar28,0,*(undefined4 *)(param_3 + 0x3c));
            uVar37 = (ulong)*(uint *)(param_3 + 0x34);
            if (1 < *(uint *)(param_3 + 0x34)) {
              uVar48 = 1;
              puVar49 = (undefined4 *)(param_3 + 0x44);
              do {
                ppcVar36 = *(code ***)
                            (*(long *)(param_3 + 0x28) + (ulong)(uVar43 + (int)uVar48) * 8);
                iVar32 = (**ppcVar36)(ppcVar36,uVar28,0,*puVar49);
                puVar1 = puVar49 + -1;
                uVar48 = uVar48 + 1;
                uVar37 = (ulong)*(uint *)(param_3 + 0x34);
                puVar49 = puVar49 + 2;
                uVar31 = iVar32 << (ulong)(*puVar1 & 0x1f) | uVar31;
              } while (uVar48 < uVar37);
            }
            piVar39 = piVar38;
            if ((((iVar35 == 2) || ((iVar35 == 0) != ((uVar31 & uVar17) != uVar5))) &&
                (DVar12 == (DensityMapFilter)0x0)) &&
               ((uVar7 == 0 || ((uVar31 >> (ulong)(uVar4 & 0x1f) & uVar16 & uVar7) != 0)))) {
              lVar45 = *(long *)param_4;
              uVar28 = ((uVar24 >> (ulong)(local_94 & 0x1f)) +
                       (uVar22 >> (ulong)(local_c0 & 0x1f)) * uVar27) * uVar29;
              uVar48 = (ulong)(uVar28 >> 3) & 0xffff;
              lVar40 = *(long *)(lVar45 + (ulong)(uVar28 >> 0x13) * 8);
              if ((uint)uVar48 < 0xfffd) {
                uVar43 = *(uint *)(lVar40 + uVar48);
              }
              else {
                uVar43 = (uVar28 >> 3) + 1;
                uVar31 = (uVar28 >> 3) + 2;
                uVar43 = (uint)CONCAT12(*(undefined *)
                                         (*(long *)(lVar45 + (ulong)(uVar31 >> 0x10) * 8) +
                                         ((ulong)uVar31 & 0xffff)),
                                        CONCAT11(*(undefined *)
                                                  (*(long *)(lVar45 + (ulong)(uVar43 >> 0x10) * 8) +
                                                  ((ulong)uVar43 & 0xffff)),
                                                 *(undefined *)(lVar40 + uVar48)));
              }
              uVar28 = uVar43 >> (ulong)(uVar28 & 7) & local_a8;
              if (((iVar8 != 2) && ((iVar8 == 0) == ((uVar28 & uVar19) != uVar9))) ||
                 ((DVar13 == (DensityMapFilter)0x0 ||
                  ((uVar30 != 0 && ((uVar28 >> (ulong)(uVar26 & 0x1f) & uVar18 & uVar30) == 0))))))
              goto LAB_007cd9bc;
              uVar28 = (uVar24 + uVar22 * uVar21) * uVar23;
              lVar45 = *(long *)param_2;
              uVar48 = (ulong)(uVar28 >> 3) & 0xffff;
              lVar40 = *(long *)(lVar45 + (ulong)(uVar28 >> 0x13) * 8);
              if ((uint)uVar48 < 0xfffd) {
                uVar43 = *(uint *)(lVar40 + uVar48);
              }
              else {
                uVar43 = (uVar28 >> 3) + 1;
                uVar31 = (uVar28 >> 3) + 2;
                uVar43 = (uint)CONCAT12(*(undefined *)
                                         (*(long *)(lVar45 + (ulong)(uVar31 >> 0x10) * 8) +
                                         ((ulong)uVar31 & 0xffff)),
                                        CONCAT11(*(undefined *)
                                                  (*(long *)(lVar45 + (ulong)(uVar43 >> 0x10) * 8) +
                                                  ((ulong)uVar43 & 0xffff)),
                                                 *(undefined *)(lVar40 + uVar48)));
              }
              uVar28 = uVar43 >> (ulong)(uVar28 & 7) & ~(-1 << (ulong)(uVar23 & 0x1f)) & 0xffffU;
              uVar43 = uVar28 & *(uint *)(param_6 + 0x20);
              uVar28 = uVar28 >> (ulong)(*(uint *)(param_6 + 0x14) & 0x1f) &
                       *(uint *)(param_6 + 0x1c);
              *(int *)(param_6 + 4) = *(int *)(param_6 + 4) + 1;
              *(uint *)(param_6 + 0x40) = uVar28;
              *(uint *)(param_6 + 0x44) = uVar43;
              if ((*(int *)(param_6 + 0x24) == 2) ||
                 ((*(int *)(param_6 + 0x24) == 0) != (*(uint *)(param_6 + 0x28) != uVar43))) {
                if ((int)uVar28 < *(int *)(param_6 + 0x2c)) {
                  GVar14 = param_6[0x34];
joined_r0x007cdadc:
                  if (GVar14 == (GetDensityProcessor)0x0) goto LAB_007cd9c8;
                }
                else {
                  GVar14 = param_6[0x34];
                  if (*(int *)(param_6 + 0x30) < (int)uVar28) goto joined_r0x007cdadc;
                  if (GVar14 != (GetDensityProcessor)0x0) goto LAB_007cd9c8;
                }
                if ((*(uint *)(param_6 + 0x38) == 0) || ((*(uint *)(param_6 + 0x38) & uVar28) != 0))
                {
                  *(uint *)param_6 =
                       (*(int *)(param_6 + 0xc) + uVar28 &
                       ((int)(*(int *)(param_6 + 0xc) + uVar28) >> 0x1f ^ 0xffffffffU)) +
                       *(int *)param_6;
                  piVar39 = (int *)(param_6 + 8);
                  goto LAB_007cd9bc;
                }
              }
            }
            else {
LAB_007cd9bc:
              *piVar39 = *piVar39 + 1;
            }
LAB_007cd9c8:
            uVar24 = uVar24 + 1;
          } while (uVar24 < *(uint *)(param_5 + 0x5c));
        }
        uVar22 = uVar22 + 1;
      } while (uVar22 < *(uint *)(param_5 + 0x4c));
    }
  }
  return true;
LAB_007cdfcc:
  do {
    uVar44 = 0;
    uVar3 = uVar42;
    do {
      uVar37 = (ulong)(uVar3 >> 3) & 0xffff;
      lVar45 = *(long *)(lVar40 + (ulong)(uVar3 >> 0x13) * 8);
      if ((uint)uVar37 < 0xfffd) {
        uVar46 = *(uint *)(lVar45 + uVar37);
      }
      else {
        uVar46 = (uVar3 >> 3) + 1;
        uVar2 = (uVar3 >> 3) + 2;
        uVar46 = (uint)CONCAT12(*(undefined *)
                                 (*(long *)(lVar40 + (ulong)(uVar2 >> 0x10) * 8) +
                                 ((ulong)uVar2 & 0xffff)),
                                CONCAT11(*(undefined *)
                                          (*(long *)(lVar40 + (ulong)(uVar46 >> 0x10) * 8) +
                                          ((ulong)uVar46 & 0xffff)),*(undefined *)(lVar45 + uVar37))
                               );
      }
      uVar46 = uVar46 >> (ulong)(uVar3 & 7) & ~(-1 << (ulong)(uVar29 & 0x1f)) & 0xffffU;
      if ((iVar8 == 2) || ((iVar8 == 0) != ((uVar46 & uVar19) != uVar9))) {
        uVar46 = uVar46 >> (ulong)(uVar26 & 0x1f) & uVar18;
        if (((int)uVar46 < iVar10) || (iVar11 < (int)uVar46)) {
          if (DVar13 != (DensityMapFilter)0x0) goto LAB_007ce094;
        }
        else if (DVar13 == (DensityMapFilter)0x0) {
LAB_007ce094:
          if ((uVar30 == 0) || ((uVar46 & uVar30) != 0)) {
            uVar41 = (uVar28 + uVar22 * uVar21) * uVar23;
            lVar45 = *(long *)param_2;
            uVar37 = (ulong)(uVar41 >> 3) & 0xffff;
            lVar40 = *(long *)(lVar45 + (ulong)(uVar41 >> 0x13) * 8);
            if ((uint)uVar37 < 0xfffd) {
              uVar42 = *(uint *)(lVar40 + uVar37);
            }
            else {
              uVar42 = (uVar41 >> 3) + 1;
              uVar44 = (uVar41 >> 3) + 2;
              uVar42 = (uint)CONCAT12(*(undefined *)
                                       (*(long *)(lVar45 + (ulong)(uVar44 >> 0x10) * 8) +
                                       ((ulong)uVar44 & 0xffff)),
                                      CONCAT11(*(undefined *)
                                                (*(long *)(lVar45 + (ulong)(uVar42 >> 0x10) * 8) +
                                                ((ulong)uVar42 & 0xffff)),
                                               *(undefined *)(lVar40 + uVar37)));
            }
            uVar41 = uVar42 >> (ulong)(uVar41 & 7) & ~(-1 << (ulong)(uVar23 & 0x1f)) & 0xffffU;
            uVar42 = uVar41 & *(uint *)(param_6 + 0x20);
            uVar41 = uVar41 >> (ulong)(*(uint *)(param_6 + 0x14) & 0x1f) & *(uint *)(param_6 + 0x1c)
            ;
            *(int *)(param_6 + 4) = *(int *)(param_6 + 4) + 1;
            *(uint *)(param_6 + 0x40) = uVar41;
            *(uint *)(param_6 + 0x44) = uVar42;
            if ((*(int *)(param_6 + 0x24) != 2) &&
               ((*(int *)(param_6 + 0x24) == 0) == (*(uint *)(param_6 + 0x28) != uVar42)))
            goto LAB_007cddc8;
            if ((int)uVar41 < *(int *)(param_6 + 0x2c)) {
              GVar14 = param_6[0x34];
joined_r0x007ce198:
              if (GVar14 == (GetDensityProcessor)0x0) goto LAB_007cddc8;
            }
            else {
              GVar14 = param_6[0x34];
              if (*(int *)(param_6 + 0x30) < (int)uVar41) goto joined_r0x007ce198;
              if (GVar14 != (GetDensityProcessor)0x0) goto LAB_007cddc8;
            }
            if ((*(uint *)(param_6 + 0x38) != 0) && ((*(uint *)(param_6 + 0x38) & uVar41) == 0))
            goto LAB_007cddc8;
            *(uint *)param_6 =
                 (*(int *)(param_6 + 0xc) + uVar41 &
                 ((int)(*(int *)(param_6 + 0xc) + uVar41) >> 0x1f ^ 0xffffffffU)) + *(int *)param_6;
            piVar38 = (int *)(param_6 + 8);
            goto LAB_007cddbc;
          }
        }
      }
      uVar44 = uVar44 + 1;
      uVar3 = uVar3 + uVar29;
    } while (uVar44 != uVar24);
    uVar41 = uVar41 + 1;
    uVar42 = uVar42 + uVar27 * uVar29;
  } while (uVar41 != uVar31);
LAB_007cddbc:
  *piVar38 = *piVar38 + 1;
LAB_007cddc8:
  uVar28 = uVar28 + 1;
  if (*(uint *)(param_5 + 0x5c) <= uVar28) goto LAB_007cdd44;
  goto LAB_007cddd8;
}


