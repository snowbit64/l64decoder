...
/* std::__ndk1::deque<ThreadPool::Task*, std::__ndk1::allocator<ThreadPool::Task*>
   >::erase(std::__ndk1::__deque_iterator<ThreadPool::Task*, ThreadPool::Task* const*,
   ThreadPool::Task* const&, ThreadPool::Task* const* const*, long, 512l>) */

void std::__ndk1::deque<ThreadPool::Task*,std::__ndk1::allocator<ThreadPool::Task*>>::erase
...

{
  ...
  if (*(long *)(uVar5 + 0x10) == lVar3) {
    lVar9 = 0;
    if (in_x2 == 0) goto LAB_00ba4600;
  }
  else {
    lVar9 = *plVar2 + (uVar8 & 0x1ff) * 8;
    if (in_x2 == lVar9) goto LAB_00ba4600;
  }
