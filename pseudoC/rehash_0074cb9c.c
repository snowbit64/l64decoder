...
/* std::__ndk1::__hash_table<std::__ndk1::__hash_value_type<std::__ndk1::pair<TransformGroup const*,
   ...
   TransformGroup const*>, std::__ndk1::vector<CollisionEffectManager::EffectInstance,
   std::__ndk1::allocator<CollisionEffectManager::EffectInstance> > > > >::rehash(unsigned long) */
...

{
  ...
  uVar2 = *(ulong *)(this + 8);
  uVar1 = param_1;
  if (uVar2 < param_1) goto LAB_0074cc50;
  if (uVar2 <= param_1) {
    return;
  }
...
LAB_0074cc3c:
    ...
    if (1 < (uint)CONCAT62((int6)((ulong)uVar4 >> 0x10),uVar3)) goto LAB_0074cc3c;
    if (1 < uVar1) {
