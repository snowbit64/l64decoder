// Ghidra Decompiler Output - AArch64 (64-bit)
// Function: FUN_007bd0b8
// Entry Point: 007bd0b8
// Size: 68 bytes
// Signature: undefined FUN_007bd0b8(void)


void FUN_007bd0b8(long param_1,long param_2)

{
  undefined4 uVar1;
  undefined4 uVar2;
  undefined4 uVar3;
  undefined4 uVar4;
  undefined4 uVar5;
  
  uVar1 = *(undefined4 *)(param_1 + 0xc);
  uVar2 = *(undefined4 *)(param_1 + 0x10);
  uVar3 = *(undefined4 *)(param_1 + 0x14);
  uVar4 = *(undefined4 *)(param_1 + 0x18);
  uVar5 = *(undefined4 *)(param_1 + 0x1c);
  *(undefined4 *)(param_2 + 0x100) = *(undefined4 *)(param_1 + 8);
  *(undefined4 *)(param_2 + 0x110) = uVar1;
  *(undefined4 *)(param_2 + 0x108) = 4;
  *(undefined4 *)(param_2 + 0x118) = 4;
  *(undefined4 *)(param_2 + 0x120) = uVar2;
  *(undefined4 *)(param_2 + 0x128) = 4;
  *(undefined4 *)(param_2 + 0x130) = uVar3;
  *(undefined4 *)(param_2 + 0x138) = 4;
  *(undefined4 *)(param_2 + 0x140) = uVar4;
  *(undefined4 *)(param_2 + 0x148) = 4;
  *(undefined4 *)(param_2 + 0x150) = uVar5;
  *(undefined4 *)(param_2 + 0x158) = 4;
  return;
}


