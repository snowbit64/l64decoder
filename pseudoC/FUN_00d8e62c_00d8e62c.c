// Ghidra Decompiler Output - AArch64 (64-bit)
// Function: FUN_00d8e62c
// Entry Point: 00d8e62c
// Size: 904 bytes
// Signature: undefined FUN_00d8e62c(void)


long FUN_00d8e62c(long *param_1,long *param_2,uint param_3,uint param_4,char param_5)

{
  uint uVar1;
  uint uVar2;
  code **ppcVar3;
  ulong uVar4;
  uint uVar5;
  ulong uVar6;
  ulong uVar7;
  long lVar8;
  int iVar9;
  long lVar10;
  long lVar11;
  ulong uVar12;
  
  uVar7 = (ulong)param_3;
  uVar1 = param_4 + param_3;
  if (((*(uint *)(param_2 + 1) < uVar1) || (*(uint *)(param_2 + 2) < param_4)) || (*param_2 == 0)) {
    ppcVar3 = (code **)*param_1;
    *(undefined4 *)(ppcVar3 + 5) = 0x17;
    (**ppcVar3)(param_1);
    uVar5 = *(uint *)((long)param_2 + 0x1c);
    if (uVar5 <= param_3) goto LAB_00d8e6b8;
LAB_00d8e6c8:
    if (*(char *)((long)param_2 + 0x26) == '\0') {
      ppcVar3 = (code **)*param_1;
      *(undefined4 *)(ppcVar3 + 5) = 0x47;
      (**ppcVar3)(param_1);
    }
    if (*(char *)((long)param_2 + 0x25) != '\0') {
      uVar5 = *(uint *)((long)param_2 + 0x14);
      if (uVar5 != 0) {
        uVar6 = (ulong)*(uint *)((long)param_2 + 0x1c);
        uVar4 = (ulong)*(uint *)(param_2 + 3);
        if ((ulong)uVar5 <= (ulong)*(uint *)(param_2 + 3)) {
          uVar4 = (ulong)uVar5;
        }
        if ((long)(*(uint *)(param_2 + 4) - uVar6) <= (long)uVar4) {
          uVar4 = *(uint *)(param_2 + 4) - uVar6;
        }
        if ((long)(*(uint *)(param_2 + 1) - uVar6) <= (long)uVar4) {
          uVar4 = *(uint *)(param_2 + 1) - uVar6;
        }
        if (0 < (long)uVar4) {
          uVar12 = 0;
          lVar8 = (ulong)*(uint *)((long)param_2 + 0xc) * 0x80;
          lVar10 = lVar8 * uVar6;
          do {
            lVar11 = uVar4 * lVar8;
            (*(code *)param_2[7])
                      (param_1,param_2 + 6,*(undefined8 *)(*param_2 + uVar12 * 8),lVar10,lVar11);
            uVar4 = (ulong)*(uint *)(param_2 + 3);
            uVar12 = uVar12 + uVar4;
            if (*(uint *)((long)param_2 + 0x14) <= uVar12) break;
            uVar6 = *(uint *)((long)param_2 + 0x14) - uVar12;
            lVar10 = lVar11 + lVar10;
            if ((long)uVar6 <= (long)uVar4) {
              uVar4 = uVar6;
            }
            lVar11 = uVar12 + *(uint *)((long)param_2 + 0x1c);
            uVar6 = (ulong)*(uint *)(param_2 + 4) - lVar11;
            if ((long)uVar6 <= (long)uVar4) {
              uVar4 = uVar6;
            }
            uVar6 = (ulong)*(uint *)(param_2 + 1) - lVar11;
            if ((long)uVar6 <= (long)uVar4) {
              uVar4 = uVar6;
            }
          } while (0 < (long)uVar4);
        }
      }
      *(undefined *)((long)param_2 + 0x25) = 0;
    }
    if (*(uint *)((long)param_2 + 0x1c) < param_3) {
      uVar2 = *(uint *)((long)param_2 + 0x14);
      uVar4 = (ulong)uVar2;
      *(uint *)((long)param_2 + 0x1c) = param_3;
    }
    else {
      uVar2 = *(uint *)((long)param_2 + 0x14);
      uVar4 = (ulong)uVar2;
      uVar5 = (uint)(uVar1 - uVar4);
      if ((long)(uVar1 - uVar4) < 1) {
        uVar5 = 0;
      }
      uVar7 = (ulong)uVar5;
      *(uint *)((long)param_2 + 0x1c) = uVar5;
    }
    if (uVar2 != 0) {
      uVar6 = (ulong)*(uint *)(param_2 + 3);
      if (uVar4 <= *(uint *)(param_2 + 3)) {
        uVar6 = uVar4;
      }
      if ((long)(*(uint *)(param_2 + 4) - uVar7) <= (long)uVar6) {
        uVar6 = *(uint *)(param_2 + 4) - uVar7;
      }
      if ((long)(*(uint *)(param_2 + 1) - uVar7) <= (long)uVar6) {
        uVar6 = *(uint *)(param_2 + 1) - uVar7;
      }
      if (0 < (long)uVar6) {
        uVar4 = 0;
        lVar8 = (ulong)*(uint *)((long)param_2 + 0xc) * 0x80;
        lVar10 = lVar8 * uVar7;
        do {
          lVar11 = uVar6 * lVar8;
          (*(code *)param_2[6])
                    (param_1,param_2 + 6,*(undefined8 *)(*param_2 + uVar4 * 8),lVar10,lVar11);
          uVar6 = (ulong)*(uint *)(param_2 + 3);
          uVar4 = uVar4 + uVar6;
          if (*(uint *)((long)param_2 + 0x14) <= uVar4) break;
          uVar7 = *(uint *)((long)param_2 + 0x14) - uVar4;
          lVar10 = lVar11 + lVar10;
          if ((long)uVar7 <= (long)uVar6) {
            uVar6 = uVar7;
          }
          lVar11 = uVar4 + *(uint *)((long)param_2 + 0x1c);
          uVar7 = (ulong)*(uint *)(param_2 + 4) - lVar11;
          if ((long)uVar7 <= (long)uVar6) {
            uVar6 = uVar7;
          }
          uVar7 = (ulong)*(uint *)(param_2 + 1) - lVar11;
          if ((long)uVar7 <= (long)uVar6) {
            uVar6 = uVar7;
          }
        } while (0 < (long)uVar6);
      }
    }
  }
  else {
    uVar5 = *(uint *)((long)param_2 + 0x1c);
    if (param_3 < uVar5) goto LAB_00d8e6c8;
LAB_00d8e6b8:
    if (*(int *)((long)param_2 + 0x14) + uVar5 < uVar1) goto LAB_00d8e6c8;
  }
  uVar5 = *(uint *)(param_2 + 4);
  if (uVar5 < uVar1) {
    if (uVar5 < param_3) {
      uVar5 = param_3;
      if (param_5 != '\0') {
        ppcVar3 = (code **)*param_1;
        *(undefined4 *)(ppcVar3 + 5) = 0x17;
        (**ppcVar3)(param_1);
        goto LAB_00d8e8f8;
      }
LAB_00d8e908:
      if (*(char *)((long)param_2 + 0x24) == '\0') {
        ppcVar3 = (code **)*param_1;
        *(undefined4 *)(ppcVar3 + 5) = 0x17;
        (**ppcVar3)(param_1);
        goto LAB_00d8e968;
      }
LAB_00d8e910:
      uVar2 = uVar5 - *(int *)((long)param_2 + 0x1c);
      if (uVar2 < uVar1 - *(int *)((long)param_2 + 0x1c)) {
        uVar1 = *(uint *)((long)param_2 + 0xc);
        lVar10 = (ulong)uVar2 << 3;
        iVar9 = (param_4 + param_3) - uVar5;
        do {
          memset(*(void **)(*param_2 + lVar10),0,(ulong)uVar1 << 7);
          lVar10 = lVar10 + 8;
          iVar9 = iVar9 + -1;
        } while (iVar9 != 0);
      }
      goto LAB_00d8e958;
    }
    if (param_5 == '\0') goto LAB_00d8e908;
LAB_00d8e8f8:
    *(uint *)(param_2 + 4) = uVar1;
    if (*(char *)((long)param_2 + 0x24) != '\0') goto LAB_00d8e910;
  }
  else {
LAB_00d8e958:
    if (param_5 == '\0') goto LAB_00d8e968;
  }
  *(undefined *)((long)param_2 + 0x25) = 1;
LAB_00d8e968:
  return *param_2 + (ulong)(param_3 - *(int *)((long)param_2 + 0x1c)) * 8;
}


