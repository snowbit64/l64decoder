		...
		if task ~= nil then
			...
			if task.cloudData ~= nil then
				...
				if self.weather.skyBoxUpdater ~= nil then
					local rainScale = 0.5 < task.cloudData.precipitation and 1 or 0

					self.weather.skyBoxUpdater:update(10000, self.dayTime, rainScale, Weather.RAIN_FADE_IN)
				end
			end
			...
			if filename ~= nil then
				...
				renderEnvProbe(renderResolution, outputResolution, task.ssaoQuality, task.cloudQuality, numIterations, filename)
			end
