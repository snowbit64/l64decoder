...
function VehicleLoadingUtil.loadVehicle(filename, location, save, price, propertyState, ownerFarmId, configurations, savegameData, asyncCallbackFunction, asyncCallbackObject, asyncCallbackArguments, registerVehicle, forceServer)
	...
	if configurations ~= nil and savegameData ~= nil and savegameData.xmlFile ~= 0 then
		savegameData.xmlFile:iterate(savegameData.key .. ".configuration", function(_, key)
	...
	if storeItem.bundleInfo ~= nil then
		...
		if storeItem.bundleInfo.bundleItems[1] ~= nil then
			if location.x == nil or location.z == nil then
				local firstBundleItem = storeItem.bundleInfo.bundleItems[1]
				local size = StoreItemUtil.getSizeValues(firstBundleItem.xmlFilename, "vehicle", firstBundleItem.rotationOffset[2])
				local places = location:getResetPlaces()
				local x, _, z, place, width, offset = PlacementUtil.getPlace(places, size, g_currentMission.usedLoadPlaces, true, false, true)
			...
			while savegameData.xmlFile:hasProperty(savegameData.key .. ".configuration(0)") do
				savegameData.xmlFile:removeProperty(savegameData.key .. ".configuration(0)")
			end

			for _, bundleItem in ipairs(storeItem.bundleInfo.bundleItems) do
				local dirX, dirZ = MathUtil.getDirectionFromYRotation(location.yRot or 0)
				local itemConfigurations = {}

				for name, index in pairs(bundleItem.item.configurations) do
					if configurations ~= nil and configurations[name] ~= nil then
						itemConfigurations[name] = configurations[name]
					end
				end

				VehicleLoadingUtil.loadVehicle(bundleItem.xmlFilename, location, save, price, propertyState, ownerFarmId, itemConfigurations, savegameData, asyncCallbackFunction, asyncCallbackObject, {}, registerVehicle, forceServer)
			end
		else
			asyncCallbackFunction(asyncCallbackObject, nil, VehicleLoadingUtil.VEHICLE_LOAD_ERROR, asyncCallbackArguments)
		end
