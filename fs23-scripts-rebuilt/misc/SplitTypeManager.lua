...
SplitTypeManager = {COLLISIONMASK_TRIGGER = 16777216}
...
function SplitTypeManager:addSplitType(name, l10nKey, splitTypeIndex, pricePerLiter, woodChipsPerLiter, allowsWoodHarvester, customEnvironment, volumeToLiter)
  ...
  local v9 = name:upper()
  if self.typesByName[v9] ~= nil then
    Logging.error("SplitTypeManager:addSplitType(): SplitType name '%s' is already in use", v9)
    return
  end
  local v10 = v10:getText(l10nKey, customEnvironment)
  self.typesByIndex[splitTypeIndex] = {name = name, title = v10, splitTypeIndex = splitTypeIndex, pricePerLiter = pricePerLiter, woodChipsPerLiter = woodChipsPerLiter, allowsWoodHarvester = allowsWoodHarvester, volumeToLiter = volumeToLiter or 1000}
  self.typesByName[name] = {name = name, title = v10, splitTypeIndex = splitTypeIndex, pricePerLiter = pricePerLiter, woodChipsPerLiter = woodChipsPerLiter, allowsWoodHarvester = allowsWoodHarvester, volumeToLiter = volumeToLiter or 1000}
end
...
function SplitTypeManager:getSplitTypeIndexByName(name)
  ...
  if self.typesByName[v2] == nil then
    return nil
  end
  return v2.splitTypeIndex
end
local v1 = SplitTypeManager.new()
g_splitTypeManager = v1
