...
function ExtendedSowingMachineRateEvent.emptyNew(v0)
  return Event.new(u0)
end
...
function ExtendedSowingMachineRateEvent:run(connection)
  ...
  if self.object ~= nil then
    ...
    if v2 then
      v2:setSeedRateAutoMode(self.automaticMode, true)
      if not self.automaticMode then
        v2:setManualSeedRate(self.manualValue, true)
      end
    end
  end
end
function ExtendedSowingMachineRateEvent.sendEvent(object, automaticMode, manualValue, noEventSend)
  ...
  if g_server ~= nil then
    local v6 = ExtendedSowingMachineRateEvent.new(object, automaticMode, manualValue)
    v4:broadcastEvent(v6, nil, nil, object)
    return
  end
  local v4 = v4:getServerConnection()
  v6 = ExtendedSowingMachineRateEvent.new(object, automaticMode, manualValue)
  v4:sendEvent(...)
end
