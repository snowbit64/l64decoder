...
function AdsSystem:draw()
	if self.renderOcclusionRegions then
		for _, group in pairs(self.groups) do
			if adsGetIsOcclusionRegionGroupActive(group.id) then
				for i, region in ipairs(group.regions) do
					local minX, minY, maxX, maxY = adsGetOcclusionRegion(group.id, i - 1)
