...
function AchievementManager:load()
	...
	while true do
		...
		local imageSizeStr = getXMLString(xmlFile, key .. "#imageSize")
		local imageSize = GuiUtils.get2DArray(imageSizeStr, {2048, 2048})
		local imageUVsStr = getXMLString(xmlFile, key .. "#imageUVs")
		local imageUVs = GuiUtils.getUVs(Utils.getNoNil(imageUVsStr, "0 0 1 1"), imageSize)
		...
		if id ~= nil and idName ~= nil then
			...
			-- Substituições de unidades de medida e moeda se necessário (baseado na lógica do FS23)
			description = string.gsub(description, "$unit", g_i18n:getMeasuringUnit(true))
			description = string.gsub(description, "$currency", g_i18n:getCurrencySymbol(true))
...
function AchievementManager:loadMapData()
	...
	for _, fillType in ipairs(g_fillTypeManager:getFillTypes()) do
		if fillType.achievementName ~= nil and self.achievementListByName[fillType.achievementName] ~= nil then
			local entry = {
				name = fillType.achievementName,
				fillType = fillType,
				startValue = 0
			}
			if not g_currentMission.missionInfo.isNewSPCareer then
				entry.startValue = fillType.totalAmount
			end
			table.insert(self.fillTypeAchievements, entry)
		end
	end
...
function AchievementManager:loadAchievementsState(showGui)
	...
	for _, achievement in pairs(self.achievementList) do
		...
		if achievement.unlocked then
			if showGui and not hasNativeAchievementGUI() then
